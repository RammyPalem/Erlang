class erlang_install {
  ...
  exec { 'check-erlang-version':
    ...
    onlyif  => "test -x /usr/lib/erlang/bin/erl",
  }
  ...
  exec { 'install-erlang':
    ...
    unless   => "test -x /usr/lib/erlang/bin/erl && /usr/lib/erlang/bin/erl -eval 'io:format(\"~s\", [erlang:system_info(otp_release)]), halt().' -noshell | grep -q $desired_version",
